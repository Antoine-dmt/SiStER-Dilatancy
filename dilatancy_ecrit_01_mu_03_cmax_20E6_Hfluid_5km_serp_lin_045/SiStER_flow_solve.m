...
for pit=1:PARAMS.Npicard_max
    ...
    %---------------------------------------------------------------------------------
    [L, R, Kc, Kb]=SiStER_assemble_L_R(dx,dy,Zs.*etas,Zn.*etan,rho,BC,PARAMS,srhs_xx,srhs_xy,R_n); %G.Ito
...
epsII_n=sqrt((EXX-0.5*R_n).^2+EXY_n.^2);%correcting volumetric augmentation for deviatoric calculation
[R_s]=SiStER_interp_normal_to_shear_nodes(R_n,dx,dy);%passing R to shear nodes
epsII_s=sqrt((EXX_s-0.5*R_s).^2+EXY.^2);
