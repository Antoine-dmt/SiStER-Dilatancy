...
if (PARAMS.YNPlas==1)
    ...
    pnn=p; 
    pnn(p<0)=0;
    yield_s=(Cohes_s+Mu_s.*ps.*(1-PARAMS.lambda_f*fc_s)).*cos(atan(Mu_s));
    yield_n=(Cohes_n+Mu_n.*pnn.*(1-PARAMS.lambda_f*fc_n)).*cos(atan(Mu_n));
...
if PARAMS.YNPlas==1 
    ...
    if PARAMS.YNElast == 1 
        %ep_dot = epsII_n;
        ep_dot = max(epsII_n-(yield_n-sqrt(sxxOLD.^2+sxyOLD_n.^2))./(2.*Gn.*dt_m),min(epsII_n(:))*1e-6);
    else 
        ep_dot = max(epsII_n,min(epsII_n(:))*1e-6); 
    end
    %
    if t >1
        R_n(n_nodes_yield) = 2 .* ep_dot(n_nodes_yield).*sind(psi_n(n_nodes_yield));
    end
