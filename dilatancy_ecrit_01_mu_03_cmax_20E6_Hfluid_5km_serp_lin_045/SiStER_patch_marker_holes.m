function [xm, ym, im, Ifix, mp, ep, idm, Tm, sxxm, sxym, epNH, epsIIm,xim,fcm]=SiStER_patch_marker_holes(icn,jcn,quad,Nx,Ny,Mquad,Mquad_crit,xm,ym,x,y,dx,dy,im,ep,idm,Tm,sxxm,sxym,epNH,epsIIm,xim,fcm)
...
sxy_fix=[]; % stress
sr_fix=[]; % strain rate
xim_fix =[];%serpentinization rate
fcm_fix = [];%fluid circulation
...
% if that was to happen, let's just draw "im" randomly
    ...
    if isempty((ep(icn==icell & jcn==jcell)))==1  
        ...
        sr_fix=zeros(1,Nfix);
        xim_fix = zeros(1,Nfix);
        fcm_fix = zeros(1,Nfix);
    else
        ...
        % serpentinization rate
        serp_fix = mean((xim(icn==icell & jcn==jcell)));
        %fluid fix
        fluid_fix = round(mode((im(icn==icell & jcn==jcell))));
    ...
    xim_fix = [xim_fix serp_fix*ones(1,Nfix)];
    fcm_fix = [fcm_fix fluid_fix*ones(1,Nfix)];
    end
