...
%GET DILATANCY
[psim]=SiStER_get_dilatancy(im,ep,MAT); % friction depends on plastic strain
[n2interp] = SiStER_interp_markers_to_normal_nodes(xm,ym,icn,jcn,x,y,psim);
psi_n=n2interp(1).data;

%FLUID CIRCULATION PHASE
[fcm] = SiStER_locate_fluids(xm,ym,Tm,topo_x,topo_y,PARAMS,xsize,im); 
[n2interp] = SiStER_interp_markers_to_normal_nodes(xm,ym,icn,jcn,x,y,fcm);
fc_n=n2interp(1).data;
[n2interp] = SiStER_interp_markers_to_shear_nodes(xm,ym,icn,jcn,qd,x,y,fcm);
fc_s = n2interp(1).data;  
fc_n = round(fc_n);
fc_s = round(fc_s);
...
[fric]=SiStER_get_friction(im,ep,MAT,xim); % friction depends on plastic strain and now max friction depends on serp rate
