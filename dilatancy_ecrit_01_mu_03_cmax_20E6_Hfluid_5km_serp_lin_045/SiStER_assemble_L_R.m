function [L, R, Kc, Kb]=SiStER_assemble_L_R(dx,dy,etas,etan,rho,BC,PARAMS,srhs_xx,srhs_xy,R_n)
...
% Computing dilatancy term Z*eta*R
dil_term = etan .* R_n;
...
% 
            ...
            % RHS
            R(inp,1)=Kc* R_n(i,j);%-drhodt(i,j);%including dilatancy here with source term R_n
        ...
        if ((j==1) && (i<=Ny-1)) || ((j==Nx) && (i<=Ny-1)) || (i==1 && j<=Nx-1 && j>=2) || (i==Ny-1 && j<=Nx-1 && j>=2) || (i==Ny)
            ...
            R(invx,1)=-gx*(rho(i,j)+rho(i+1,j))/2 - ...
                ...
                + 2.*(dil_term(i+1,j+1)-dil_term(i+1,j))/(dx(j)+dx(j-1));
                %dilatancy correction added to right hand side
        end
        ...
        if (j==1 && i<=Ny-1 && i>=2) || (j==Nx-1 && i<=Ny-1 && i>=2) || (j==Nx) || (i==1 && j<=Nx-1) || (i==Ny && j<=Nx-1)
            ...
            R(invy,1)=-gy*(rho(i,j)+rho(i,j+1))/2 + ...
                ...
                2.*(dil_term(i+1,j+1)-dil_term(i,j+1))/(dy(i)+dy(i-1));
            % dilatancy correction added to Y_STOKES right hand side
        end
